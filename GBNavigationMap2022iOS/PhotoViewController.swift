...
import UIKit
import AVFoundation
// Подключаем фреймворк для работы с медиа import AVFoundation
class PhotoViewController: UIViewController {
    ...
    override func viewWillTransition(to size: CGSize, with coordinator: UIViewControllerTransitionCoordinator) {
        ...
        if previewLayer?.connection?.isVideoOrientationSupported == true { // Устанавливаем текущую ориентацию, иначе изображение будет перевёрнутым
    ...
    override func viewDidLoad() { super.viewDidLoad()
        ...
        self.cameraOutput = cameraOutput // Создаём сессию видеозахвата
    ...
    @IBAction func takePicture(_ sender: Any) { // Создаём настройки получения снимка
    ...
    func cameraDeviceFind() -> AVCaptureDevice? { // Настройки поиска
    ...
    mediaType: .video,
    position: .front
        // Обычная передняя камера
        // Тип устройства – камера
        // Передняя камера
    )
    ...
    func makeCameraOutput() -> AVCapturePhotoOutput {
        // Создаём объект, который будет получать данные из сессии видеозахвата // в момент получения снимка
        let cameraOutput = AVCapturePhotoOutput() // Здесь мы можем указать различные настройки, // в документации их описано намного больше
        // Высокое разрешение
        cameraOutput.isHighResolutionCaptureEnabled = true // Живые фото отключены
    ...
    func makeCameraSession(cameraOutput: AVCapturePhotoOutput) -> AVCaptureSession? {
        ...
        guard
            ...
            let input = try? AVCaptureDeviceInput(device: device), // Если можем добавить в сессию в качестве источника данных
        ...
        captureSession.sessionPreset = AVCaptureSession.Preset.photo // Добавляем объект, отвечающий за получение фотоданных
    ...
    func configurePreviewLayer(_ captureSession: AVCaptureSession, _ cameraOutput: AVCapturePhotoOutput) {
        ...
        let previewLayer = AVCaptureVideoPreviewLayer(session: captureSession) // Добавляем его на корневую вьюшку экрана
...
extension PhotoViewController: AVCapturePhotoCaptureDelegate {
    func photoOutput(_ output: AVCapturePhotoOutput, didFinishProcessingPhoto
        ...
        guard
            ...
            let imageData = photo.fileDataRepresentation(), // Если смогли сделать из них изображение
            let uiImage = UIImage(data: imageData), // Если извлекли кор изображения
            let cgImage = uiImage.cgImage, // Если есть запомненная ориентация
...
fileprivate extension UIDeviceOrientation {
    ...
    func getUIImageOrientationFromDevice() -> UIImage.Orientation {
        ...
        switch self { case .portrait,
                .faceUp: orientation = .right
        case .portraitUpsideDown, .faceDown:
            orientation = .left case .landscapeLeft:
            orientation = .down case .landscapeRight: orientation = .up
        case .unknown: orientation = .down
        }
        return orientation }
    // Преобразование текущей ориентации экрана в ориентацию видеозахвата
    func getAVCaptureVideoOrientationFromDevice() -> AVCaptureVideoOrientation { let orientation: AVCaptureVideoOrientation
        switch self {
        case .portrait,
                .faceUp:
